...
const SuggestedBox: FC<SuggestBoxProps> = ({ content, label, icon, item }) => {
  ...
  return (
    <>
      <button onClick={handleClick} className="max-w-[266px] w-full h-28 p-3 bg-white flex rounded-md border border-[#e9e9e9] flex-col justify-start items-start gap-2">
        {!!label?.length && <div className={cn("self-stretch text-start text-black text-sm font-semibold  leading-tight", (isTeacher || isCareerCoach) ? "min-h-[20px] max-h-[40px] overflow-hidden text-ellipsis line-clamp-2" : "")}>{icon} {label}</div>}
